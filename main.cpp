/*
 * vulkan_stereo_3dvision.cpp  v4
 *
 * Stereoscopic 3D – NVIDIA 3D Vision beta driver 426.06, GeForce/consumer GPU.
 *
 * ── DIAGNOSIS OF PREVIOUS FAILURES ──────────────────────────────────────────
 *
 *  v2/v3 both returned VK_ERROR_INITIALIZATION_FAILED (-3) from
 *  vkCreateSwapchainKHR with imageArrayLayers=2, whether FSE was chained
 *  into the swapchain CI or not.  FSE presence/absence in the CI is NOT
 *  the cause.
 *
 *  Root cause: in v3 VK_EXT_full_screen_exclusive was still LOADED ON THE
 *  DEVICE even though it was not chained into the swapchain CI.  On driver
 *  426.06, having the FSE device extension active while creating a stereo
 *  (imageArrayLayers=2) swapchain blocks the driver's internal stereo
 *  initialisation path and returns -3.  The 3D Vision driver manages
 *  exclusive display ownership through its own mechanism; FSE must not be
 *  present on the device when stereo is active.
 *
 * ── SWAPCHAIN PATHS ──────────────────────────────────────────────────────────
 *
 *  STEREO  (3D Vision ON  -->  maxImageArrayLayers == 2):
 *    - FSE extension NOT loaded on device.
 *    - imageArrayLayers = 2,  pNext = null.
 *    - Present mode: IMMEDIATE preferred.  The 3D Vision shutter circuit runs
 *      at 120 Hz; FIFO's internal queue can slip frames relative to the IR
 *      sync signal, causing the wrong eye to receive the wrong frame.
 *    - The 3D Vision driver takes exclusive display ownership internally.
 *
 *  NON-STEREO  (3D Vision OFF  -->  maxImageArrayLayers == 1):
 *    - FSE extension loaded, APPLICATION_CONTROLLED.
 *    - vkAcquireFullScreenExclusiveModeEXT called after swapchain creation.
 *    - imageArrayLayers = 1.  Only left-eye (cyan) pass executes.
 *    - Present mode: FIFO.
 *
 * ── BUILD ────────────────────────────────────────────────────────────────────
 ...
 *   Log written to vulkan_stereo.log beside the exe, flushed per line.
 *   Press ESC to quit.
 */
...
// ---------------------------------------------------------------------------
// Logging – every line flushed so a hard crash still leaves a complete log
// ---------------------------------------------------------------------------
...
// Throws on any non-SUCCESS result.
...
// Tolerates SUBOPTIMAL, OUT_OF_DATE, FSE_LOST; throws on anything else.
...
static bool hasInstanceExt(const char *name) {
    ...
    for (auto &e : v) if (!strcmp(e.extensionName, name)) return true;
    return false;
}
...
// WS_POPUP covering the full primary monitor.
// Required for both paths:
//   Stereo:     3D Vision driver needs to flip without DWM compositing.
//   Non-stereo: vkAcquireFullScreenExclusiveModeEXT requires full coverage.
...
struct StereoApp {
    ...
    // stereo=true  --> imageArrayLayers=2, NO FSE loaded, 3D Vision path
    // stereo=false --> imageArrayLayers=1, FSE loaded,    non-stereo path
    bool stereo = false;
    ...
    // Extension state – both determined BEFORE device creation
    bool hasSurfaceCaps2 = false;
    bool hasFSE          = false;   // always false when stereo==true
    ...
    void init(HINSTANCE hInst, HWND hwnd) {
        ...
        pickPhysicalDevice();   // sets stereo + hasFSE
        createDevice();         // loads FSE only when !stereo
    ...
    // -----------------------------------------------------------------------
    // CRITICAL: stereo and hasFSE are determined HERE, before device creation.
    // When stereo==true, hasFSE is forced to false so the FSE device extension
    // is never loaded – its mere presence on the device causes -3 on 426.06.
    void pickPhysicalDevice() {
        ...
        for (auto pd : devs) {
            ...
            // Decode NVIDIA packed driver version (top-10 bits = major, next-8 = minor)
            uint32_t drvMaj = props.driverVersion >> 22;
            uint32_t drvMin = (props.driverVersion >> 14) & 0xFF;
            log("  GPU: %s  driver %u.%u (raw 0x%x)",
                props.deviceName, drvMaj, drvMin, props.driverVersion);
            ...
            for (uint32_t i=0; i<qfc; ++i) {
                ...
                log("  stereo=%s", stereo?"YES":"NO");

                if (stereo) {
                    // Stereo path: FSE must NOT be loaded on the device.
                    // Having VK_EXT_full_screen_exclusive as a device extension
                    // (even unused in swapchain CI) causes vkCreateSwapchainKHR
                    // to return VK_ERROR_INITIALIZATION_FAILED (-3) on 426.06.
                    // The 3D Vision driver manages exclusive display ownership
                    // through its own internal mechanism.
                    hasFSE = false;
                    log("  STEREO PATH: FSE will NOT be loaded on device");
                } else {
                    bool fseAvail = hasSurfaceCaps2 &&
                        hasDeviceExt(pd, VK_EXT_FULL_SCREEN_EXCLUSIVE_EXTENSION_NAME);
                    hasFSE = fseAvail;
                    log("  NON-STEREO PATH: FSE device ext %s",
                        hasFSE ? "will be loaded" : "NOT available");
                }
                break;
            }
            if(physDev) break;
    ...
    void createDevice() {
        ...
        if(hasFSE) {
            ...
            log("  Loading VK_EXT_full_screen_exclusive on device");
        } else {
            log("  VK_EXT_full_screen_exclusive NOT loaded (stereo=%s)",
                stereo?"yes – intentionally omitted":"no – not available");
        }
        ...
        if(hasFSE) {
            ...
            log("  pfnAcquireFSE: %s", pfnAcquireFSE?"loaded":"NOT loaded");
    ...
    void createSwapchain(HWND hwnd) {
        ...
        // Stereo: IMMEDIATE preferred – 3D Vision shutter sync requires the driver
        //         to schedule the flip at the exact 120 Hz boundary; FIFO's queue
        //         latency can push a flip past that boundary and swap the eyes.
        // Non-stereo: FIFO (vsync, no tearing).
        uint32_t pmCount=0;
        ...
        if(stereo) {
            for(auto pm:pms)
                if(pm==VK_PRESENT_MODE_IMMEDIATE_KHR){
                    presentMode=VK_PRESENT_MODE_IMMEDIATE_KHR; break;
                }
            log("  Stereo present mode: %s",
                presentMode==VK_PRESENT_MODE_IMMEDIATE_KHR
                    ? "IMMEDIATE"
                    : "FIFO (IMMEDIATE not available -- stereo sync may be unreliable)");
        } else {
            log("  Non-stereo present mode: FIFO");
        }
        ...
        ci.imageArrayLayers = layers;    // 2 = stereo (layer 0=left, layer 1=right)
        ...
        ci.clipped          = VK_TRUE;
        // ci.pNext = null for stereo path (FSE not loaded; driver manages exclusivity)

        // Non-stereo only: attach FSE chain
        ...
        if(!stereo && hasFSE) {
            ...
            ci.pNext = &fseInfo;
            log("  Non-stereo: FSE chain attached (APPLICATION_CONTROLLED)");
        }

        log("  vkCreateSwapchainKHR: layers=%u  presentMode=%d  pNext=%s",
            layers, (int)presentMode, ci.pNext ? "FSE-chain" : "null");

        VK_CHECK(vkCreateSwapchainKHR(device,&ci,nullptr,&swapchain));

        // Non-stereo: explicitly acquire exclusive ownership
        if(!stereo && hasFSE && pfnAcquireFSE) {
            VkResult r=pfnAcquireFSE(device,swapchain);
            log("  vkAcquireFullScreenExclusiveModeEXT: %s (%d)",
                vkResultStr(r),(int)r);
        }
    ...
    // -----------------------------------------------------------------------
    // Two render passes, both with initialLayout=UNDEFINED.
    // UNDEFINED means "discard prior content" which is always valid per spec
    // regardless of the image's actual layout -- no barrier needed between passes.
    ...
    void recordCommandBuffer(uint32_t idx) {
        ...
        // Left eye: CYAN (R=0 G=1 B=1) -- always rendered
        {
            ...
            // layer 0 now in PRESENT_SRC_KHR
        }

        // Right eye: RED (R=1 G=0 B=0) -- STEREO ONLY
        // Skipping in non-stereo prevents the second clear from overwriting layer 0
        // (which caused "only red shown" in earlier versions of this demo).
        if(stereo) {
            VkClearValue cv{};
            ...
            vkCmdEndRenderPass(cb);
            // layer 1 now in PRESENT_SRC_KHR
        }
    ...
    void drawFrame() {
        ...
        if(r==VK_ERROR_OUT_OF_DATE_KHR ||
           r==VK_ERROR_FULL_SCREEN_EXCLUSIVE_MODE_LOST_EXT) return;
...
int WINAPI WinMain(HINSTANCE hInst, HINSTANCE, LPSTR, int) {
    ...
    logInit();
    log("=== VkStereo3DVision v4 startup ===");
